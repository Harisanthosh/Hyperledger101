...
describe('Starting tests for yes-network', function() {
  ...
  it('website title should be yes-network', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('yes-network');
    })
  });

  it('network-name should be yes@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('yes@0.0.1.bna');
    });
  });

  it('navbar-brand should be yes-network',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('yes-network');
    });
  });
